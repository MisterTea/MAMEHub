...
public class NewAccountDialog extends JDialog {
	...
	public NewAccountDialog(JFrame parent, final RpcEngine rpcEngine) {
		...
		{
			JLabel lblNewLabel = new JLabel("Fill out some information to get started");
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if(pr == RpcEngine.PingResponse.SERVER_DOWN) {
							JOptionPane.showMessageDialog(NewAccountDialog.this, "New Account failed: MAMEHub server is down.");
							return;
						}
						if(pr == RpcEngine.PingResponse.CLIENT_TOO_OLD) {
							JOptionPane.showMessageDialog(NewAccountDialog.this, "New Account failed: Your client is out of date, please restart MAMEHub.");
							return;
						}
						if(!passwordOnceTextField.getText().equals(passwordTwiceTextField.getText())) {
							JOptionPane.showMessageDialog(NewAccountDialog.this, "Passwords much match.");
							return;
						}
						if(emailTextField.getText().length()<3 || passwordOnceTextField.getText().length()<3) {
							JOptionPane.showMessageDialog(NewAccountDialog.this, "Please enter a username & password with at least 3 characters.");
							return;
						}
						if(!usernameTextField.getText().matches("[0-9a-zA-Z]+")) {
							JOptionPane.showMessageDialog(NewAccountDialog.this, "Usernames must contain only letters and numbers.");
							return;
						}
						if(!EmailValidator.getInstance().isValid(emailTextField.getText())) {
							JOptionPane.showMessageDialog(NewAccountDialog.this, "Please enter a valid email address.");
							return;
						}
						
						String errorMessage = rpcEngine.newAccount(emailTextField.getText(), usernameTextField.getText(), passwordOnceTextField.getText());
