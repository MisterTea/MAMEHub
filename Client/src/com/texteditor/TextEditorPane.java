...
public class TextEditorPane extends JTextArea implements Serializable {
	...
	public void findForward(String word)
	{
		...
		{
			JOptionPane.showMessageDialog(this, word + " doesn't exist in this document", "Sorry", JOptionPane.INFORMATION_MESSAGE);
		}
	...
	public void printPane()
	{
		...
		int n = (int) (pj.defaultPage().getImageableHeight() - pj.defaultPage().getImageableY())/getFont().getSize();
		Vector v = new Vector();
		for (int i = 0; i<lines.size(); i++)
		{
			v.add(lines.get(i));
		...
		try {
			if (pj.printDialog())
			{
				...
				try
				{
					...
					JOptionPane.showMessageDialog(null, "Error in the printing of the document", "Print Error", JOptionPane.ERROR_MESSAGE);
				}
			}
		} catch (Exception e)
		{
	...
	private ArrayList<String> getLinesToPrint(String atext)
	{
		...
		while (i < letters.length)
		{
			if (getFontMetrics(getFont()).charsWidth(temp.toCharArray(), 0, temp.toCharArray().length) + getFontMetrics(getFont()).charWidth(letters[i]) > width)
			{
				lines.add(temp);
				temp = "";
				temp += letters[i];
