...
void ui_manager::update_and_render(render_container *container)
{
	...
	if(waitingForClientCatchup && false)
	{
        char buf[4096];
        sprintf(buf,"A new client is joining, please wait...");
        draw_text_box(container,buf,JUSTIFY_CENTER,0.5f,0.5f,rgb_t(255,0,0,128));
	}
	else if(
        ...
        )
    {
        ...
        draw_text_box(container,"This could take several minutes depending on your connection and rom chosen...",JUSTIFY_CENTER,0.5f,0.6f,rgb_t(255,0,0,128));
        draw_text_box(container,"Once the initial sync is complete, you may just hear game audio for a few minutes, please be patient",JUSTIFY_CENTER,0.5f,0.7f,rgb_t(255,0,0,128));
        char buf[4096];
	...
	static const rgb_t chatColors[] =
        {
            ...
            rgb_t(192,0,128,128),
            rgb_t(192,0,0,0),
            rgb_t(192,128,128,128),
            rgb_t(192,128,128,255),
            rgb_t(192,128,255,128),
            rgb_t(192,255,255,128),
            rgb_t(192,128,255,128),
            rgb_t(192,255,128,128),
        };
	for(
	    list<ChatLog>::iterator it = chatLogs.begin();
	    it != chatLogs.end();
                                ...
                                if(chatString.size()>1 && chatString[1]>='1' && chatString[1]<='9') {
                                  ...
                                  netCommon->setPlayer(chatString[1]-'1');
                                    ...
                                    if(netServer->isBlockNewClients())
                                        machine.ui().popup_time(3, "Game is locked and new clients cannot join.");
                                    else
                                        machine.ui().popup_time(3, "Game is unlocked, new clients can join.");
                                    ...
                                    machine.ui().popup_time(3, "Captured %d locations",(int)locationsToIntersect.size());
