...
public class ClientDatabaseEngine {
  ...
  public static class ThriftSerializer<T extends TBase> implements
  ...
  private final static Logger logger = Logger
      .getLogger(ClientDatabaseEngine.class.getName());
  ...
  public ClientDatabaseEngine(String baseDbDirectory, String dbName,
      boolean wipe, boolean inMemory, boolean compression) throws IOException {
    super();
  ...
  @SuppressWarnings("rawtypes")
  public synchronized <T extends TBase> ConcurrentMap<String, T> getOrCreateMap(
      Class<T> inClass, String className) {
    ConcurrentMap<String, T> dbMap = null;
    if (dbMap == null) {
      dbMap = database.getTreeMap(className);
      if (dbMap == null) {
        dbMap = database.createTreeMap(className).nodeSize(120).counterEnable()
            .valueSerializer(new ThriftSerializer<T>(inClass)).makeStringMap();
      }
    }
    return dbMap;
  }

  public synchronized <T> ConcurrentMap<String, T> getOrCreatePrimitiveMap(
      String className) {
    ConcurrentMap<String, T> dbMap = null;
    if (dbMap == null) {
      dbMap = database.getTreeMap(className);
