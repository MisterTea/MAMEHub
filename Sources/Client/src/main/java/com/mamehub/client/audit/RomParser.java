...
public class RomParser extends DefaultHandler {
  ...
  void process(Map<String, ArrayList<RomHashEntryValue>> hashEntryMap2,
    ...
    try {
      ...
      saxParser.parse(new GZIPInputStream(new FileInputStream(xmlFile)), this);
    ...
    // always true.
    /*
     * for (String romId : new HashSet<String>(roms.keySet())) { RomInfo romInfo
     * = new RomInfo(roms.get(romId)); if (romInfo.cloneRom != null &&
     * roms.get(romInfo.cloneRom).missingReason != null) { romInfo.missingReason
     * = MR.MISSING_CLONE; } else if (romInfo.parentRom != null &&
     * roms.get(romInfo.parentRom).missingReason != null) {
     * romInfo.missingReason = MR.MISSING_PARENT; } roms.put(romId, romInfo); }
     */
    this.roms = null;
  }
  ...
  public void startElement(String uri, String localName, String qName,
    ...
    if (qName.equals("game") || qName.equals("machine")) {
      ...
      if (attributes.getValue("ismechanical") != null
          && attributes.getValue("ismechanical").equalsIgnoreCase("yes")) {
        if (verbose) {
          System.out.println("GAME IS MECHANICAL: "
              + attributes.getValue("ismechanical"));
        }
      ...
      if (attributes.getValue("sha1") == null) {
        ...
        if (chdMap != null && chdMap.containsKey(attributes.getValue("sha1"))) {
          romInfo.filenames.add(chdMap.get(attributes.getValue("sha1")));
        } else {
          if (verbose)
            System.out.println("MISSING CHD: " + attributes.getValue("name")
                + " " + attributes.getValue("sha1"));
          chdFailed = true;
        }
      }
    } else if (qName.equals("softwarelist")) {
      romInfo.softwareLists
          .add(new SoftwareList().setName(attributes.getValue("name"))
              .setFilter(attributes.getValue("filter")));
      ...
      if (entries == null) {
        if (verbose) {
          logger.info(matchFileName + " GOT 0 FOR " + romInfo._id + " : "
              + name);
        }
        gameFailed = true;
        return;
      ...
      if (matchFileName) {
        for (RomHashEntryValue f : entries) {
          if (verbose) {
            logger.info("" + f.location + " " + romInfo._id.toLowerCase());
          }
  ...
  public void characters(char ch[], int start, int length) throws SAXException {
    ...
    if (nextDataIsDescription) {
      ...
      romInfo.description = romInfo.description.concat(new String(ch, start,
          length));
    }
  ...
  public void endElement(String uri, String localName, String qName)
    ...
    if (qName.equals("game") || qName.equals("machine")) {
      ...
      } else if (romsWithNoHash == 0 && goodRoms == 0 && allowZeroFileRoms) {
        // This is ok
