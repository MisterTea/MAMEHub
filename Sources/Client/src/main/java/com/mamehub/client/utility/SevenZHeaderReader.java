...
public class SevenZHeaderReader {
  ...
  static final byte[] sevenZSignature = { (byte) '7', (byte) 'z', (byte) 0xBC,
  ...
  private StartHeader readStartHeader(final int startHeaderCrc)
    ...
    try {
      ...
      startHeader.nextHeaderOffset = Long.reverseBytes(dataInputStream
          .readLong());
      startHeader.nextHeaderSize = Long
          .reverseBytes(dataInputStream.readLong());
  ...
  private void readSubStreamsInfo(final DataInput header, final Archive archive)
      throws IOException {
    ...
    for (final Folder folder : archive.folders) {
      ...
      subStreamsInfo.unpackSizes[nextUnpackStream++] = folder.getUnpackSize()
          - sum;
    }
    if (nid == NID.kSize) {
      nid = header.readUnsignedByte();
  ...
  private void readFilesInfo(final DataInput header, final Archive archive)
        ...
        if (external != 0) {
          ...
          for (int i = 0; i < names.length; i += 2) {
            if (names[i] == 0 && names[i + 1] == 0) {
              files[nextFile++].setName(new String(names, nextName, i
                  - nextName, CharsetNames.UTF_16LE));
              nextName = i + 2;
            }
          }
        ...
        if (external != 0) {
          ...
          for (int i = 0; i < files.length; i++) {
            ...
            if (files[i].getHasLastModifiedDate()) {
              files[i]
                  .setLastModifiedDate(Long.reverseBytes(header.readLong()));
            }
          }
        }
        break;
        ...
        if (external != 0) {
          ...
          for (int i = 0; i < files.length; i++) {
            ...
            if (files[i].getHasWindowsAttributes()) {
              files[i].setWindowsAttributes(Integer.reverseBytes(header
                  .readInt()));
            }
          }
        }
    ...
    for (int i = 0; i < files.length; i++) {
      files[i].setHasStream((isEmptyStream == null) ? true : !isEmptyStream
          .get(i));
      if (files[i].hasStream()) {
        ...
        files[i]
            .setSize(archive.subStreamsInfo.unpackSizes[nonEmptyFileCounter]);
        ++nonEmptyFileCounter;
      } else {
        files[i].setDirectory((isEmptyFile == null) ? true : !isEmptyFile
