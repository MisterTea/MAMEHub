...
public class MameHubEngine implements Runnable {
  ...
  public MameHubEngine(GameAuditor.AuditHandler ah, EmulatorHandler eh)
      throws IOException {
    this.handler = eh;

    mediaCommandFetcher = new MediaCommandFetcher(
        Utils.getResourceReader("/media.txt"));

    gameAuditor = new GameAuditor(ah);

    Runtime.getRuntime()
        .addShutdownHook(new Thread(new ExitMameShutdownHook()));
  ...
  public boolean launchGame(String username, String systemName,
      RomInfo romOrCart, boolean isServer, String ipAddress, int selfport,
    ...
    if (systemName.equalsIgnoreCase("arcade")) {
      ...
      // Before we passed the bios path to ume, now we need to pass the system
      // name.
      // this.romFileName = getMessRomInfo(systemName).filename;
      this.romFileName = systemName;
      cmdList.add(romFileName);
      for (Entry<String, String> interfaceTypeNamePair : romOrCart.interfaceFileMap
          .entrySet()) {
        cmdList.add("-" + interfaceTypeNamePair.getKey());
        // cmdList.add(interfaceTypeNamePair.getValue());
        cmdList.add(romOrCart._id);
      }
    }
  ...
  public void run() {
    ...
    File outputFile = new File("../logs/" + new File(romFileName).getName()
        + "_" + (System.currentTimeMillis() / 1000) % (60 * 24 * 365) + ".txt")
        .getAbsoluteFile();
