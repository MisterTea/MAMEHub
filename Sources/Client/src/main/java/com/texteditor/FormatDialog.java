...
/**
 * 
 * class for the creation and visualization of a dialog with formatting features
...
public class FormatDialog extends JDialog implements ActionListener {
  ...
  private void createComponent() {
    ...
    container.add(new JLabel(""));
    container.add(new JLabel(""));
    labelStyle = new JLabel("Style", SwingConstants.CENTER);
    container.add(labelStyle);
    comboStyle = new JComboBox(
        new String[] { "Normale", "Grassetto", "Corsivo" });
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (source.equals(buttonCancel)) {
      ...
      Color color = JColorChooser.showDialog(this, "Choose the Color",
          labelColor.getBackground());
      if (color != null)
        labelColor.setBackground(color);
    } else if (source.equals(buttonDone) || source.equals(buttonApply)) {
      textPane.setFormat(
          new Font(comboFont.getSelectedItem().toString(), comboStyle
              .getSelectedIndex(), Integer.parseInt(comboSize.getSelectedItem()
              .toString())), labelColor.getBackground());
