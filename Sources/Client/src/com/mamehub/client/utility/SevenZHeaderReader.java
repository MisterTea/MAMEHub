 ...
 * @since 1.6
 */
public class SevenZHeaderReader {
	...
	class Folder {
		// / List of coders used in this folder, eg. one for compression, one
		// for encryption.
		Coder[] coders;
		...
		BindPair[] bindPairs;
		// / Indeces of input streams, one per input stream not listed in
		// bindPairs.
		long[] packedStreams;
		...
		// / The CRC, if present.
		int crc;
		// / The number of unpack substreams, one per non-empty file in this
		// folder.
	...
	class Archive {
		...
		// / CRCs for each packed stream, valid only if that packed stream has
		// one.
		int[] packCrcs;
	...
	static final byte[] sevenZSignature = { (byte) '7', (byte) 'z',
			(byte) 0xBC, (byte) 0xAF, (byte) 0x27, (byte) 0x1C };
	 ...
	 *            the file to read
	 * @param password
	 *            optional password if the archive is encrypted - the byte array
	 *            is supposed to be the UTF16-LE encoded representation of the
	 *            password.
	 * @throws IOException
	 *             if reading the archive fails
	 */
	public SevenZHeaderReader(final File filename, final byte[] password)
			throws IOException {
	 ...
	 *             if reading the archive fails
	 */
	public SevenZHeaderReader(final File filename) throws IOException {
		this(filename, null);
	}
	...
	private Archive readHeaders(byte[] password) throws IOException {
		...
		if (archiveVersionMajor != 0) {
			throw new IOException(String.format(
					"Unsupported 7z version (%d,%d)", archiveVersionMajor,
					archiveVersionMinor));
		}
		...
		if (nid == NID.kEncodedHeader) {
			nextHeaderInputStream = readEncodedHeader(nextHeaderInputStream,
					archive, password);
	...
	private StartHeader readStartHeader(final int startHeaderCrc)
			throws IOException {
		...
		try {
			dataInputStream = new DataInputStream(
					new CRC32VerifyingInputStream(
							new BoundedRandomAccessFileInputStream(file, 20),
							20, startHeaderCrc));
			startHeader.nextHeaderOffset = Long.reverseBytes(dataInputStream
					.readLong());
			startHeader.nextHeaderSize = Long.reverseBytes(dataInputStream
					.readLong());
			startHeader.nextHeaderCrc = Integer.reverseBytes(dataInputStream
					.readInt());
			return startHeader;
		} finally {
	...
	private void readHeader(final DataInput header, final Archive archive)
			throws IOException {
	...
	private void readArchiveProperties(final DataInput input)
			throws IOException {
	...
	private DataInputStream readEncodedHeader(final DataInputStream header,
			final Archive archive, byte[] password) throws IOException {
		readStreamsInfo(header, archive);
		...
		InputStream inputStreamStack = new BoundedRandomAccessFileInputStream(
				file, archive.packSizes[firstPackStreamIndex]);
	...
	private void readStreamsInfo(final DataInput header, final Archive archive)
			throws IOException {
		int nid = header.readUnsignedByte();
	...
	private void readPackInfo(final DataInput header, final Archive archive)
			throws IOException {
		...
		if (nid == NID.kCRC) {
			archive.packCrcsDefined = readAllOrBits(header,
					(int) numPackStreams);
			archive.packCrcs = new int[(int) numPackStreams];
			for (int i = 0; i < (int) numPackStreams; i++) {
				if (archive.packCrcsDefined.get(i)) {
					archive.packCrcs[i] = Integer
							.reverseBytes(header.readInt());
				}
			}
	...
	private void readUnpackInfo(final DataInput header, final Archive archive)
			throws IOException {
		int nid = header.readUnsignedByte();
		if (nid != NID.kFolder) {
	...
	private void readSubStreamsInfo(final DataInput header,
		...
		for (final Folder folder : archive.folders) {
			...
			subStreamsInfo.unpackSizes[nextUnpackStream++] = folder
					.getUnpackSize() - sum;
		}
		if (nid == NID.kSize) {
			nid = header.readUnsignedByte();
	...
	private BitSet readAllOrBits(final DataInput header, final int size)
			throws IOException {
	...
	private BitSet readBits(final DataInput header, final int size)
			throws IOException {
	...
	private void readFilesInfo(final DataInput header, final Archive archive)
			throws IOException {
		...
		while (true) {
				...
				if (external != 0) {
					...
					for (int i = 0; i < files.length; i++) {
						...
						if (files[i].getHasCreationDate()) {
							files[i].setCreationDate(Long.reverseBytes(header
									.readLong()));
						}
					}
				}
				...
				if (external != 0) {
					...
					for (int i = 0; i < files.length; i++) {
						...
						if (files[i].getHasAccessDate()) {
							files[i].setAccessDate(Long.reverseBytes(header
									.readLong()));
						}
					}
				}
				...
				if (external != 0) {
					...
					for (int i = 0; i < files.length; i++) {
						...
						if (files[i].getHasLastModifiedDate()) {
							files[i].setLastModifiedDate(Long
									.reverseBytes(header.readLong()));
						}
					}
				}
				break;
			...
			case NID.kWinAttributes: {
				...
				if (external != 0) {
					...
					for (int i = 0; i < files.length; i++) {
						files[i].setHasWindowsAttributes(attributesDefined
								.get(i));
						if (files[i].getHasWindowsAttributes()) {
							files[i].setWindowsAttributes(Integer
									.reverseBytes(header.readInt()));
						}
					}
				}
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (files[i].hasStream()) {
				...
				files[i].setHasCrc(archive.subStreamsInfo.hasCrc
						.get(nonEmptyFileCounter));
				...
				files[i].setAntiItem((isAnti == null) ? false : isAnti
						.get(emptyFileCounter));
	...
	private void calculateStreamMap(final Archive archive) throws IOException {
		...
		final int numFolders = (archive.folders != null) ? archive.folders.length
				: 0;
	...
	private void buildDecodingStream() throws IOException {
		...
		if (currentFolderIndex == folderIndex) {
			...
			currentFolderInputStream = buildDecoderStack(folder, folderOffset,
					firstPackStreamIndex);
		}
		final SevenZArchiveEntry file = archive.files[currentEntryIndex];
		final InputStream fileStream = new BoundedInputStream(
				currentFolderInputStream, file.getSize());
		if (file.getHasCrc()) {
			currentEntryInputStream = new CRC32VerifyingInputStream(fileStream,
					file.getSize(), file.getCrc());
		} else {
	...
	private InputStream buildDecoderStack(final Folder folder,
			final long folderOffset, final int firstPackStreamIndex)
			throws IOException {
		file.seek(folderOffset);
		InputStream inputStreamStack = new BoundedRandomAccessFileInputStream(
				file, archive.packSizes[firstPackStreamIndex]);
